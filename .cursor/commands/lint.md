---
description: Fix all linting and formatting issues across the project. Follow the fixing workflow when resolving issues.
---
...
2. **Auto-fix**
   - In utils: `npm run check:fix` (Biome lint + format with fix). From root: `make utils-check-fix`.
   - Or run `npm run lint -- --write` and `npm run format` to fix what can be auto-fixed.
...
4. **Fix issues systematically**
   ...
   - Verify with: `npx biome check <file>` (utils) or project linter.
