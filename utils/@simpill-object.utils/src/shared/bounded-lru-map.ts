import {
  ERROR_MAX_SIZE_MUST_BE_GREATER_THAN_ZERO,
  NAME_BOUNDED_LRU_MAP,
  VALUE_0,
  VALUE_0_8,
} from "./constants";
...
export class BoundedLRUMap<K, V> {
  ...
  constructor(options: BoundedLRUMapOptions | number) {
    if (typeof options === "number") {
      ...
      this.alertThreshold = VALUE_0_8;
      this.name = NAME_BOUNDED_LRU_MAP;
      this.enableAlerts = true;
      this.logger = undefined;
    } else {
      this.maxSize = options.maxSize;
      this.alertThreshold = options.alertThreshold ?? VALUE_0_8;
      this.name = options.name ?? NAME_BOUNDED_LRU_MAP;
      this.enableAlerts = options.enableAlerts ?? true;
      this.logger = options.logger;
    }

    if (this.maxSize <= VALUE_0) {
      throw new Error(ERROR_MAX_SIZE_MUST_BE_GREATER_THAN_ZERO);
    }
