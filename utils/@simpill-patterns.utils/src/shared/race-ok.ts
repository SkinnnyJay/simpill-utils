...
import { ERROR_RACE_OK_NO_RESULT, ERROR_RACE_OK_REQUIRES_ONE } from "./constants";
...
const defaultMapError = <E>(
  ...
  const message = options?.errorMessage ?? ERROR_RACE_OK_NO_RESULT;
...
export async function raceOk<T, E>(
  ...
  if (results.length === 0) {
    return err(new AppError(ERROR_RACE_OK_REQUIRES_ONE, { code: ERROR_CODES.BAD_REQUEST }));
  }
