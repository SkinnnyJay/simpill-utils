...
for dir in "$UTILS_DIR"/@simpill-*.utils; do
  ...
  # Copy package (exclude build artifacts and deps)
  rsync -a --exclude='node_modules' --exclude='dist' --exclude='coverage' \
    --exclude='.jest-cache' --exclude='*.tgz' --exclude='.DS_Store' \
    "$dir/" "$dest/"
  ...
  node -e "
    ...
    if (j.dependencies) {
      for (const k of Object.keys(j.dependencies)) {
        ...
        if (typeof v === 'string' && v.startsWith('file:../@simpill-')) {
          j.dependencies[k] = '^1.0.0';
        }
      }
    }
