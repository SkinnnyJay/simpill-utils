...
import { ENCRYPTED_VALUE_PREFIX, ENV_ERROR_MESSAGE, ENV_PARSE_TYPE } from "../shared/constants";
import {
  EnvDecryptError,
  EnvParseError,
  EnvValidationError,
...
export function getRequiredStringFromEnv(
  getValue: GetValueFn,
  key: string,
  errorMessage?: string
): string {
...
export function getNumberFromEnv(getValue: GetValueFn, key: string, defaultValue: number): number {
...
export function isEncryptedFromEnv(rawCache: Map<string, string>, key: string): boolean {
  ...
  return raw?.startsWith(ENCRYPTED_VALUE_PREFIX) ?? false;
}

export function getDecryptedFromEnv(
  ctx: EnvGettersContext,
  key: string,
  privateKey?: string
): string {
