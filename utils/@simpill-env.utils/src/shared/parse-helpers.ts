import {
  ...
  ENV_ERROR_MESSAGE,
  ENV_PARSE_TYPE,
  ENV_VALIDATION_MUST_BE_ONE_OF_PREFIX,
} from "./constants";
...
export function parseEnvEnum<T extends string>(
  ...
  const match = allowed.find(
    (a) =>
      (options?.caseInsensitive ? a.toLowerCase() : a) ===
      (options?.caseInsensitive ? rawValue.toLowerCase() : rawValue)
  );
...
export function parseEnvEnumStrict<T extends string>(
  ...
  if (rawValue === "") {
    throw new EnvValidationError(key, rawValue, ENV_ERROR_MESSAGE.VALUE_REQUIRED);
  }
  const match = allowed.find(
    (a) =>
      (options?.caseInsensitive ? a.toLowerCase() : a) ===
  ...
  if (match === undefined) {
    throw new EnvValidationError(
      key,
      rawValue,
      `${ENV_VALIDATION_MUST_BE_ONE_OF_PREFIX}${allowed.join(", ")}`
    );
  }
  return match;
}
